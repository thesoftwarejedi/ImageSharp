...
namespace ImageSharp.Quantizers
{
    ...
    public class PaletteQuantizer : IQuantizer
    {
        ...
        /// <inheritdoc/>
        public QuantizedImage<TPixel> Quantize<TPixel>(ImageBase<TPixel> image, int maxColors)
            where TPixel : struct, IPixel<TPixel>
        {
            var quant = new PaletteQuantizerInner<TPixel>()
            {
                Dither = this.Dither
            };

            if (this.DitherType != null)
            {
                quant.DitherType = this.DitherType;
            }

            return quant.Quantize(image, maxColors);
        }

        private sealed class PaletteQuantizerInner<TPixel> : BasePixelQuantizer<TPixel>
            where TPixel : struct, IPixel<TPixel>
        {
            /// <summary>
            ...
            public PaletteQuantizerInner(TPixel[] palette = null)
                : base(true)
            {
                if (palette == null)
                {
