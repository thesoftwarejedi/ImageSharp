...
namespace ImageSharp.Processors
{
    ...
    public abstract class Matrix3x2Processor2<TColor, TPacked> : ResamplingWeightedProcessor<TColor, TPacked>
        where TColor : struct, IPackedPixel<TPacked>
        where TPacked : struct
    {
        public Matrix3x2Processor2(IResampler sampler, int width, int height, Rectangle resizeRectangle)
            : base(sampler, width, height, resizeRectangle)
        {
            this.CanvasRectangle = resizeRectangle;
        }
