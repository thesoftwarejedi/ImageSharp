...
namespace AvatarWithRoundedCorner
{
    ...
    static class Program
    {
        static void Main(string[] args)
        {
            ...
            using (IImage img = Image.Load("fb.jpg"))
            {
                // as generate returns a new IImage make sure we dispose of it
                using (IImage dest = img.Generate(x => x.ApplyAvater(new Size(200, 200), 20)))
                {
                    dest.Save("output/fb.png");
                }

                using (IImage destRound = img.Generate(x => x.ApplyAvater(new Size(200, 200), 100)))
                {
                    destRound.Save("output/fb-round.png");
                }

                using (IImage destRound = img.Generate(x => x.ApplyAvater(new Size(200, 200), 150)))
                {
        ...
        private static IImageOperations ApplyAvater(this IImageOperations operations, Size size, float cornerRadius)
        {
